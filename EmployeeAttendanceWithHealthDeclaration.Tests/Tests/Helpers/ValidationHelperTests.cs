...
namespace EmployeeAttendanceWithHealthDeclaration.Tests.Helpers
{
    public class ValidationHelperTests
    {
        ...
        #region ValidateEmployee — ordered first-error-wins chain

        [Fact]
        public void Null_employee_returns_employee_data_required()
        {
            Assert.Equal("Employee data is required", ValidationHelper.ValidateEmployee(null));
        }
        ...
        public void Blank_name_returns_name_required_before_assignment_checks()
        {
            ...
            Assert.Equal("Name is required", ValidationHelper.ValidateEmployee(employee));
        }

        [Fact]
        public void Null_assignments_returns_at_least_one_project()
        {
            var employee = new EmployeeBuilder().WithoutAssignments().Build();
        ...
        {
            var employee = new EmployeeBuilder().WithAssignments(new List<employee_project_assignment>()).Build();
        ...
        public void Fifty_one_assignments_return_max_fifty_projects()
        {
            var employee = new EmployeeBuilder()
                .WithAssignments(Enumerable.Range(1, 51)
                    .Select(i => new employee_project_assignment { project_code = $"P-{i:D3}", position = "Worker" })
                    .ToList())
                .Build();

            Assert.Equal("An employee can be assigned to at most 50 projects",
                ValidationHelper.ValidateEmployee(employee));
        }
        ...
        public void Exactly_fifty_valid_assignments_are_accepted()
        {
            ...
            Assert.Null(ValidationHelper.ValidateEmployee(employee));
        }
        ...
        {
            var employee = new EmployeeBuilder()
                .WithAssignments(new List<employee_project_assignment>
                {
        ...
        public void Blank_position_returns_position_required()
        {
            ...
            Assert.Equal("Position is required", ValidationHelper.ValidateEmployee(employee));
        }
        ...
        {
            var employee = new EmployeeBuilder().WithProviderCode("").Build();

            Assert.Equal("Provider code is required", ValidationHelper.ValidateEmployee(employee));
        }
        ...
        public void Birthdate_is_validated_last_in_the_chain()
        {
            ...
            var employee = new EmployeeBuilder().WithBirthdate(null).Build();

            Assert.Equal("Birthdate is required", ValidationHelper.ValidateEmployee(employee));
        }

        [Fact]
        public void Fully_valid_employee_returns_null()
        {
            Assert.Null(ValidationHelper.ValidateEmployee(new EmployeeBuilder().Build()));
        }
