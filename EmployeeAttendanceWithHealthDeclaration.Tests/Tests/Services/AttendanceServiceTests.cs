...
namespace EmployeeAttendanceWithHealthDeclaration.Tests.Services
{
    public class AttendanceServiceTests
    {
        ...
        private AttendanceService CreateService() => new(
            _employeeRepository, _timeLogsRepository, _providerRepository, _systemConfigService,
        ...
        private void StubActiveEmployee(string employee_id = "TST-000001", string name = "Juan Dela Cruz")
        {
            _employeeRepository.GetByEmployeeId(employee_id)
                .Returns(new EmployeeBuilder().WithEmployeeId(employee_id).WithName(name).Build());
        }

        #region ProcessScan

        [Fact]
        public async Task ProcessScan_unknown_employee_is_rejected()
        {
            _employeeRepository.GetByEmployeeId("TST-999999").Returns((employee?)null);
        ...
        [Fact]
        public async Task ProcessScan_inactive_employee_is_rejected()
        {
            _employeeRepository.GetByEmployeeId("TST-000001")
                .Returns(new EmployeeBuilder().AsInactive().Build());

            var result = await CreateService().ProcessScan("TST-000001");

            Assert.False(result.Success);
            Assert.Equal("Juan Dela Cruz - Employee is inactive", result.Message);
        }
        ...
        {
            // Open session + zero active projects + inactive provider: the employee
        ...
        public async Task ProcessScan_zero_active_projects_blocks_time_in()
        {
            ...
            _employeeRepository.GetByEmployeeId("TST-000001")
                .Returns(new EmployeeBuilder().WithActiveProjectCount(0).Build());
            _timeLogsRepository.GetLastScan("TST-000001").Returns((time_log?)null);
            _timeLogsRepository.GetTodayTimeIn("TST-000001").Returns((time_log?)null);
            _timeLogsRepository.GetOpenSession("TST-000001").Returns((time_log?)null);
