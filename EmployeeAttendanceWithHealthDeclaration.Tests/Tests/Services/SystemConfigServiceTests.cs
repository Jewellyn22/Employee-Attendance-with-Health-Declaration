...
namespace EmployeeAttendanceWithHealthDeclaration.Tests.Services
{
    public class SystemConfigServiceTests
    {
        ...
        [Fact]
        public async Task AdGroup_missing_config_falls_back_to_an_empty_group()
        {
            var result = await CreateService().GetAdminADGroup();

            Assert.False(result.Success);
            Assert.Equal(string.Empty, result.Data); // fails closed - no hardcoded group
        }

        [Fact]
        public async Task AdGroup_repository_failure_falls_back_to_an_empty_group()
        {
