...
namespace ContractorAttendanceWithHealthDeclaration.Repositories
{
    public class ContractorEmployeeRepository : IContractorEmployeeRepository
    {
        ...
        // QrCodeHelper at enrollment. sp_contractor_employee_SetQrCode touches ONLY
        ...
        {
            const string storedProc = "sp_contractor_employee_SetQrCode";
            var result = await _db.ExecuteAsync(
                storedProc,
        ...
        // Export-only read (sp_contractor_employee_GetQrCodes): returns the stored
        // QR blob per enrolled employee. Dapper maps it straight into the
        // contractor_qr_code DTO; the domain model deliberately has no blob property
        // so audit snapshots and the grid payload never carry base64.
        public async Task<IEnumerable<contractor_qr_code>> GetQrCodes()
        {
            const string storedProc = "sp_contractor_employee_GetQrCodes";
            return await _db.QueryAsync<contractor_qr_code>(
                storedProc,
                commandType: CommandType.StoredProcedure
            );
        }
