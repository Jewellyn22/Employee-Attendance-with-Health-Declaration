...
namespace ContractorAttendanceWithHealthDeclaration.Tests.Infrastructure
{
    ...
    /// rows ("N contractors") created during this fixture's lifetime.
    /// </summary>
    public static class IntegrationCleanup
    {
        public static async Task DeleteTestRowsAsync(string connectionString, DateTime startedAtUtc)
        {
            ...
            await connection.ExecuteAsync(
                "DELETE FROM contractor_project WHERE employee_id LIKE 'TST%' OR project_code LIKE 'TST%'");

            await connection.ExecuteAsync(
                "DELETE FROM contractor_employee WHERE employee_id LIKE 'TST%' OR provider_code LIKE 'TST%'");
            ...
            await connection.ExecuteAsync(
                @"DELETE FROM `audit_log`
                  WHERE reference_id LIKE 'TST%'
                     ...
                     OR (reference_id LIKE '% contractors' AND created_at >= @since)",
