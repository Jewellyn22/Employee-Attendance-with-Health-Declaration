...
namespace ContractorAttendanceWithHealthDeclaration.Tests.Repositories
{
    ...
    public class TimeLogsRepositoryIntegrationTests
    {
        ...
        public async Task Create_returns_the_inserted_row_with_id_and_created_at()
        {
            ...
            await IntegrationSeed.CreateContractorAsync(connection, employee_id: "TST-000041");
        ...
        public async Task GetLastScan_prefers_the_most_recent_time_out_over_an_older_time_in()
        {
            await using var connection = _fixture.CreateConnection();
            await IntegrationSeed.CreateProviderAsync(connection);
            await IntegrationSeed.CreateContractorAsync(connection, employee_id: "TST-000042");
            var repo = new TimeLogsRepository(connection);
        ...
        public async Task GetOpenSession_returns_only_the_row_with_no_time_out()
        {
            await using var connection = _fixture.CreateConnection();
            await IntegrationSeed.CreateProviderAsync(connection);
            await IntegrationSeed.CreateContractorAsync(connection, employee_id: "TST-000043");
            var repo = new TimeLogsRepository(connection);
        ...
        public async Task GetTodayTimeIn_ignores_yesterdays_open_session()
        {
            await using var connection = _fixture.CreateConnection();
            await IntegrationSeed.CreateProviderAsync(connection);
            await IntegrationSeed.CreateContractorAsync(connection, employee_id: "TST-000044");
            var repo = new TimeLogsRepository(connection);
        ...
        public async Task Update_persists_every_written_column()
        {
            await using var connection = _fixture.CreateConnection();
            await IntegrationSeed.CreateProviderAsync(connection);
            await IntegrationSeed.CreateContractorAsync(connection, employee_id: "TST-000045");
            var repo = new TimeLogsRepository(connection);
        ...
        public async Task UpdateHealthStatus_changes_only_the_health_column()
        {
            await using var connection = _fixture.CreateConnection();
            await IntegrationSeed.CreateProviderAsync(connection);
            await IntegrationSeed.CreateContractorAsync(connection, employee_id: "TST-000046");
            var repo = new TimeLogsRepository(connection);
        ...
        public async Task Delete_removes_the_row_and_reports_what_it_deleted()
        {
            await using var connection = _fixture.CreateConnection();
            await IntegrationSeed.CreateProviderAsync(connection);
            await IntegrationSeed.CreateContractorAsync(connection, employee_id: "TST-000047");
            var repo = new TimeLogsRepository(connection);
        ...
        public async Task GetAllFiltered_scopes_by_employee_health_and_date()
        {
            await using var connection = _fixture.CreateConnection();
            await IntegrationSeed.CreateProviderAsync(connection);
            await IntegrationSeed.CreateContractorAsync(connection, employee_id: "TST-000048");
            var repo = new TimeLogsRepository(connection);
            ...
            Assert.All(all, r => Assert.Equal("TST Contractor One", r.name));
        }
        ...
        public async Task GetAllFiltered_empty_string_filter_behaves_like_no_filter()
        {
            await using var connection = _fixture.CreateConnection();
            await IntegrationSeed.CreateProviderAsync(connection);
            await IntegrationSeed.CreateContractorAsync(connection, employee_id: "TST-000049");
            var repo = new TimeLogsRepository(connection);
        ...
        public async Task GetOpenSessionsCount_increases_by_one_for_a_new_open_session()
        {
            await using var connection = _fixture.CreateConnection();
            await IntegrationSeed.CreateProviderAsync(connection);
            await IntegrationSeed.CreateContractorAsync(connection, employee_id: "TST-000050");
            var repo = new TimeLogsRepository(connection);
        ...
        public async Task GetTodayAttendance_and_GetRecentForDashboard_include_todays_scan()
        {
            await using var connection = _fixture.CreateConnection();
            await IntegrationSeed.CreateProviderAsync(connection);
            await IntegrationSeed.CreateContractorAsync(connection, employee_id: "TST-000051");
            var repo = new TimeLogsRepository(connection);
            ...
            Assert.Equal("TST Contractor One", row.name);
        }
        ...
        public async Task GetByEmployeeId_returns_that_employees_logs()
        {
            await using var connection = _fixture.CreateConnection();
            await IntegrationSeed.CreateProviderAsync(connection);
            await IntegrationSeed.CreateContractorAsync(connection, employee_id: "TST-000052");
            var repo = new TimeLogsRepository(connection);
        ...
        public async Task GetDailyStatsByProvider_groups_counts_per_day_and_provider()
        {
            ...
            await IntegrationSeed.CreateContractorAsync(connection, employee_id: "TST-000060", provider_code: "TST-DA");
            await IntegrationSeed.CreateContractorAsync(connection, employee_id: "TST-000061", provider_code: "TST-DB");
            var repo = new TimeLogsRepository(connection);
        ...
        public async Task GetDailyStatsByProvider_excludes_rows_outside_the_window()
        {
            ...
            await IntegrationSeed.CreateContractorAsync(connection, employee_id: "TST-000062", provider_code: "TST-DC");
            var repo = new TimeLogsRepository(connection);
        ...
        // PIN: contractor_project is many-to-many — one TIME IN from a contractor
        // assigned to N projects is counted under ALL N projects.
        [Fact]
        public async Task GetDailyStatsByProject_counts_a_contractor_under_each_assigned_project()
        {
            await using var connection = _fixture.CreateConnection();
            await IntegrationSeed.CreateProviderAsync(connection);
            ...
            await IntegrationSeed.CreateContractorAsync(connection, employee_id: "TST-000063");
            ...
            // One scan, but a row for each of the contractor's two projects.
        ...
        public async Task GetDailyStatsByProject_excludes_soft_deleted_projects()
        {
            ...
            await IntegrationSeed.CreateContractorAsync(connection, employee_id: "TST-000064");
