...
namespace ContractorAttendanceWithHealthDeclaration.Tests.Repositories
{
    ...
    public class ProjectRepositoryIntegrationTests
    {
        ...
        public async Task GetByProviderCode_returns_only_that_providers_projects()
        {
            ...
            var others = await repo.Create(NewProject("TST Theirs"));
            others!.provider_code = "TSTP-OTHER";
            await repo.Update(others!);
            ...
            Assert.DoesNotContain(list, p => p.project_code == others!.project_code);
        }

        [Fact]
        public async Task Delete_soft_deletes_and_cascades_only_to_contractors_without_another_active_project()
        {
            await using var connection = _fixture.CreateConnection();
            await IntegrationSeed.CreateProviderAsync(connection);
            ...
            await IntegrationSeed.CreateContractorAsync(connection, employee_id: "TST-000011");
            await IntegrationSeed.AssignProjectAsync(connection, "TST-000011", "TST-26-901");
            await IntegrationSeed.CreateContractorAsync(connection, employee_id: "TST-000012");
            ...
            var c1 = await connection.QuerySingleAsync<int>(
                "SELECT is_deleted FROM contractor_employee WHERE employee_id = 'TST-000011'");
            var c2 = await connection.QuerySingleAsync<int>(
                "SELECT is_deleted FROM contractor_employee WHERE employee_id = 'TST-000012'");
