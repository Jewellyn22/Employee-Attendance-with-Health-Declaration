...
namespace ContractorAttendanceWithHealthDeclaration.Tests.Services
{
    public class ProjectServiceTests
    {
        ...
        private ProjectService CreateService() => new(
            _projectRepository, _providerRepository, _auditLogService, _contractorService,
        ...
        public async Task Update_can_deactivate_a_project_with_an_expired_contract()
        {
            ...
            _contractorService.CascadeDeactivateByProject("TST-26-001", Admin)
        ...
        public async Task Update_without_status_change_skips_the_cascades()
        {
            ...
            await _contractorService.DidNotReceive().CascadeDeactivateByProject(Arg.Any<string>(), Arg.Any<string>());
            await _contractorService.DidNotReceive().CascadeReactivateByProject(Arg.Any<string>(), Arg.Any<string>());
        }
        ...
        public async Task Update_deactivation_cascades_and_appends_the_count()
        {
            ...
            Assert.True(result.Success);
            Assert.Equal("Project updated successfully - 2 contractor(s) deactivated", result.Message);
            var audit = _auditCalls.Single();
        ...
        [Fact]
        public async Task Update_reactivation_restores_cascade_deactivated_contractors()
        {
            var submitted = ValidProject(active: 1);
            var existing = ValidProject(active: 0);
            _providerRepository.GetByProviderCode("TST").Returns(new provider { provider_code = "TST", active = 1 });
            _projectRepository.GetByProjectCode("TST-26-001").Returns(existing);
            _projectRepository.Update(Arg.Any<project>()).Returns(c => c.Arg<project>());
            _contractorService.CascadeReactivateByProject("TST-26-001", Admin)
                .Returns(new Response<int> { Success = true, Data = 3 });

            var result = await CreateService().Update(submitted, Admin);

            Assert.True(result.Success);
            Assert.Equal("Project updated successfully - 3 contractor(s) re-activated", result.Message);
        }
